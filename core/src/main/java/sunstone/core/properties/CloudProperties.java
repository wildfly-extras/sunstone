...
import sunstone.core.SunstoneLogger;
...
public final class CloudProperties {
    private static final Logger LOGGER = SunstoneLogger.DEFAULT;
    ...
    public CloudProperties load(Class<?> clazz) {
        if (clazz != null) {
            ...
            try (InputStream is = clazz.getResourceAsStream(propertiesResourcePath)) {
                if (is == null) {
                    throw new RuntimeException(String.format("Loading CloudProperties from classpath resource" +
